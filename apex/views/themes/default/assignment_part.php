                    ...
                    <div class="main_nav_block">
                        <div class="filter-result-sort">
                            ...
                            <select class="sortings-filt-date">
                                <option value="asc" <?php if ($this->session->userdata('sort_price_assignment') == 'asc'){?>selected<?php } ?>>цена по возрастанию</option>
                                <option value="desc" <?php if ($this->session->userdata('sort_price_assignment') == 'desc'){?>selected="selected"<?php } ?>>цена по убыванию</option>
                            </select>
                            <div class="show_right_sort">
                                <a href="#" class="<?php if ($shown['plitter_assignment'] == 'list'){?>chplit<?php } ?> plit plist" data-value="plit">Плитка</a>
                                <a href="#" class="<?php if ($shown['plitter_assignment'] == 'plit'){?>chplit<?php } ?> lisst plist" data-value="list">Список</a>
                                <a href="#res-map" class="onmap">На карте</a>
                            </div>
                        </div>
                        <?php if($empty): ?>
                        ...
                        <div class="filter-result-paging sm_selectpage headpag" data-category="SelectPage" data-label="buildings">
                            <?php if($params['page'] !== 'all' && !empty($pagination)): ?>
                                <?php echo $pagination ?>
                                <br><br><a href="/<?php echo uri(1); ?>/?<?=$show_all?>" rel="nofollow" class="filter-result-paging-link sm_search" data-category="ShowAll" data-label="buildings" style="margin-top: 10px; font-size: 13px; margin-left: 0;">Показать все переуступки</a>
                            <?php endif; ?>
                        </div>
                    </div>
                    <?php $this->load->view('themes/default/tmpl/fastsearch_assignment'); ?>
                    <?php if ($shown['plitter_assignment'] == 'plit'){?>
                                ...
                                <div class="mrow clearfix">
                                    ...
                                    <div class="onecol mcol-4<?php if($k == 4):?> nomargin<?php endif;?>">
                                        <a href="<?=$r->link?>" class="object-item sm_search" data-category="SelectObject" data-label="buildings">
                                            ...
                                            <div class="viewob">просмотр переуступки</div>
                                        </a>
                                    </div>
                                    <?php $k++; if($k > 4) { $k = 1; } ?>
                                    <?php endforeach;?>
