                        ...
                        <div class="fixed-container clearfix">
                            ...
                            <div class="block-right text-center">
                                ...
                                <?php if(!empty($in_favorites)):?>
                                    <a href="#" class="addtofavorite" data-category="land" data-id="<?php echo $rows->id; ?>" data-action="delete">Убрать из избранного</a>
                                <?php else: ?>
                                    <a href="#" class="addtofavorite notinf" data-category="land" data-id="<?php echo $rows->id; ?>" data-action="add">В избранное</a>
                                <?php endif;?>
                            </div>
                        </div>
                    </div>
    ...
    <div class="fixed-container clearfix">
        <div class="complex-info-col-left">
            ...
            <dl class="complex-info-list reslist">
                ...
                <?php if(!empty($rows->forwhat)): ?>
                    <dt>Назначение:</dt>
                    <dd><?php echo $rows->forwhat; ?>&nbsp;</dd>
                <?php endif; ?>
                <?php if(!empty($rows->tokad)): ?>
                    <dt>Расстояние до КАД:</dt>
                    <dd><?php echo $rows->tokad; ?>&nbsp;км</dd>
                <?php endif; ?>
                <?php if(!empty($rows->square)): ?>
                    <dt>Площадь:</dt>
                    <dd><?php echo $rows->square; ?>&nbsp;га</dd>
                <?php endif; ?>
            ...
            <?php if(is_admins()): ?><p><a class="complex-title" style="font-size: 14px; border: 1px solid; padding: 3px 6px; border-bottom: 0; border-top: 0;" href="/land/admin/index/edit/<?php echo $rows->id; ?>" target="_blank">Редактировать в админке</a></p><?php endif; ?>
        </div>
                    ...
                    <li class="onecol mcol-4">
                        <a href="<?php echo $see_value['link']; ?>" class="object-item sm_search" data-category="SelectObject" data-label="buildings">
                            ...
                            <div class="filter-result-item-body">
                                ...
                                <p class="filter-result-item-size"><?php echo $see_value['tokad']; ?></p>
                                <div class="addbott">
                                    <div class="filter-result-item-address"><?php echo $see_value['address']; ?></div>
                                </div>
                            </div>
...
<div id="askLand" class="mfp-hide white-popup-block askpopup" data-name="Земельные массивы | <?=$rows->header;?>" data-link="<?=getUrl();?>">
    ...
    <form id="land_form_ask">
        <div class="rowd">
            <textarea id="comment" data-question="Меня интересует земельный участок «<?=$rows->header;?>», который находится по адресу <?php echo $rows->address; ?>">Меня интересует земельный участок «<?=$rows->header;?>», который находится по адресу <?php echo $rows->address; ?></textarea>
        </div>
        <div class="rowd">
        ...
        <div class="rowd">
            <input type="submit" class="submit-ask" name="submit" value="отправить" onclick="yaCounter29760432.reachGoal('otpravit-zayav'); _gaq.push(['_trackEvent', 'OtpravitVoprosZemlya', 'Click']); return true;" />
        </div>
    </form>
</div>
