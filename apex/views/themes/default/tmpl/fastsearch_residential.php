<div class="notshow fsblock clearfix">
    ...
    <div class="fsrght">
        ...
        <div class="rayonfs clearfix fslinksblock active">
            ...
            $metro = $this->rayon_model->getAllFastFilter(2);
            $cm = count($metro);
            if ($cm > 0){
                $km = 0;
                ...
                <?php foreach ($metro as $k => $v): ?>
                    ...
                    <?php  } $km++; ?>
                    <li>
                        <?php if($link !== $v['link']): ?><a href="/residential/<?=$v['link']?>"><?php endif;?><?=$v['name']?><?php if($link !== $v['link']): ?></a><?php endif;?>
                    </li>
                <?php endforeach; ?>
                </ul>
            <?php } ?>
        ...
        <div class="typefs clearfix fslinksblock">
            <?php
            $this->load->model('residential_object/residential_object_model','residential_object');
            $metro = $this->residential_object->getAllFastFilter();
            $cm = count($metro);
            if ($cm > 0){
                $km = 0;
                $half = ($cm > 5) ? (int)ceil(($cm) / 5) : 1;
