<div class="notshow fsblock clearfix">
    ...
    <div class="fsrght">
        <div class="metrofs clearfix fslinksblock active">
            ...
            $metro = $this->metro_model->getAllFastFilter(8);
            $cm = count($metro);
            if ($cm > 0){
                $km = 0;
                ...
                <?php foreach ($metro as $k => $v): ?>
                    ...
                    <?php  } $km++; ?>
                    <li>
                        <?php if($link !== $v['link']): ?><a href="/assignment/<?=$v['link']?>"><?php endif;?><?=$v['name']?><?php if($link !== $v['link']): ?></a><?php endif;?>
                    </li>
                <?php endforeach; ?>
                </ul>
            <?php } ?>
        ...
        <div class="rayonfs clearfix fslinksblock">
            ...
            $metro = $this->rayon_model->getAllFastFilter(8);
            $cm = count($metro);
            if ($cm > 0){
                $km = 0;
                $half = ($cm > 5) ? (int)ceil(($cm) / 5) : 1;
