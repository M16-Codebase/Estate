<div class="notshow fsblock clearfix">
    ...
    <div class="fsrght">
        ...
        <div class="rayonfs clearfix fslinksblock active">
            ...
            $metro = $this->rayon_model->getAllFastFilter(3);
            $cm = count($metro);
            if ($cm > 0){
                $km = 0;
                ...
                <?php foreach ($metro as $k => $v): ?>
                    ...
                    <?php  } $km++; ?>
                    <li>
                        <?php if($link !== $v['link']): ?><a href="/elite/<?=$v['link']?>"><?php endif;?><?=$v['name']?><?php if($link !== $v['link']): ?></a><?php endif;?>
                    </li>
                <?php endforeach; ?>
                </ul>
            <?php } ?>
        ...
        <div class="typefs clearfix fslinksblock">
            ...
            $metro = $this->elite_type->getAllFastFilter();
            $cm = count($metro);
            if ($cm > 0){
                $km = 0;
                $half = ($cm > 5) ? (int)ceil(($cm) / 5) : 1;
