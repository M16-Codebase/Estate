			...
			<div class="block-complex-intro-inner">
				<div class="fixed-container clearfix">
					<div class="block-complex-intro-col">
						...
						<a class="backto" href="<?php echo $refer; ?>">Назад в раздел</a>
					</div>
					<div class="block-right text-center">
						...
						<?php } else { ?>
							<a style="display:none;" href="<?php echo $rows->presentation; ?>" title="М16-Недвижимость" target="_blank" class="pres-link">Скачать презентацию</a>
						<?php } ?>
						...
						<?php if(!empty($in_favorites)) { ?>
							<a href="#" class="addtofavorite" data-category="assignment" data-id="<?php echo $id; ?>" data-action="delete">Убрать из избранного</a>
						<?php } else { ?>
							<a href="#" class="addtofavorite notinf" data-category="assignment" data-id="<?php echo $id; ?>" data-action="add">В избранное</a>
						<?php } ?>
					</div>
				</div>
			</div>
				...
				<div class="sliderkit photosgallery-captions" id="verticali-resale">
					<div class="sliderkit-panels">
						...
						<?php if (!empty($rows->tour)) { ?>
							<div class="sliderkit-panel">
								<iframe src="<?php echo $rows->tour; ?>" frameborder="0" width="822" height="530"></iframe>
								<span class="plazhka"></span>
							</div>
						<?php } ?>
						<?php foreach($rows->foto['foto'] as $k => $v) { ?>
							<div class="sliderkit-panel">
								<img src="<?php echo image_resize($v, 'resizebig'); ?>" alt="<?php echo $rows->foto['alt'][$k]; ?>" />
								<div class="sliderkit-panel-textbox">
									<?php if ($rows->foto['alt'][$k] != '') { ?>
										<div class="sliderkit-panel-text">
											<h4><?php echo $rows->foto['alt'][$k]; ?></h4>
										</div>
					...
					<div class="sliderkit-nav">
						<div class="sliderkit-nav-clip">
							<ul>
								<?php if (!empty($rows->tour)) { ?>
									<li class="panoramali"><a href="#" rel="nofollow" title=""><img src="<?php echo image($rows->mainfoto,'bigsliderthumb'); ?>" width="114px" height="114px" alt="" /><span class="panoramacap"></span></a></li>
								<?php } ?>
								<?php foreach($rows->foto['foto'] as $k => $v) { ?>
									<li><a href="#" rel="nofollow" title="<?php echo $rows->foto['alt'][$k]; ?>"><img src="<?php echo image($v,'bigsliderthumb'); ?>" width="114px" height="114px" alt="<?php echo $rows->foto['alt'][$k]; ?>" /></a></li>
								<?php } ?>
							</ul>
						</div>
...
<section class="complex-info infnov">
	<div class="infohead">
		<ul>
			...
			<?php if (!empty($rows->otdelka)) { ?><li class="changeinfo" id="otdelka"><a href="#">Планировка</a></li><?php } ?>
			<?php if (!empty($rows->infrastruct)) { ?><li class="changeinfo" id="infrastructura"><a href="#">Дом и инфраструктура</a></li><?php } ?>
		</ul>
	</div>
	<div class="fixed-container clearfix">
		<div class="complex-info-col-left">
			...
			<?php if (!empty($rows->builder)) { ?>
				<div class="builderinfo">
					<img src="<?php echo $rows->builder['image']; ?>" />
					<b>Застройщик:</b><br>
					<?php echo $rows->builder['name']; ?>
				</div>
			<?php } ?>
			<?php if (is_admins()) { ?><p><a class="complex-title" style="font-size: 14px; border: 1px solid; padding: 3px 6px; border-bottom: 0; border-top: 0;" href="/buildings/admin/index/edit/<?php echo $id; ?>" target="_blank">Редактировать в админке</a></p><?php } ?>
		</div>
		<div class="complex-info-col-right">
			<div class="active infobl descrip">
				...
				<?php if ($rows->newObjectLink) { ?>
				<br />
				<p><a href="/buildings/<?php echo $rows->newObjectLink; ?>" target="_blank" style="color: #009ce0;">Квартиры от застройщика в ЖК &laquo;<?php echo $rows->header; ?>&raquo;</a>
				<?php } ?>
			</div>
					...
					<?php if (count($rows->foto_otdelka['foto']) > 0) { ?>
						<ul class="otd_foto">
							<?php foreach($rows->foto_otdelka['foto'] as $k => $v) { ?>
								<li><a href="<?php echo $v; ?>" class="foto_otd"><img src="<?php echo $v; ?>" width="100px" height="100px" /></a></li>
							<?php } ?>
						</ul>
					<?php } ?>
				</div>
	...
	<div class="advantage-block horizontal clearfix">
		<div class="title-advan">
			...
			<div class="gift-choose">
				фирменный подарок от вячеслава малафеева!<br>
		...
		<div id="">
			<div class="titlesadv">
				<h3 data-id="1" class="active"><span>1</span>Индивидуальный подход и<br>большой выбор объектов</h3>
				<h3 data-id="2"><span>2</span>Гарантии<br>и безопасность</h3>
				<h3 data-id="3"><span>3</span>Cервис<br>и комфорт</h3>
				<h3 data-id="4"><span>4</span>Гибкие условия сделок и<br>уникальные предложения</h3>
			</div>
			<div class="contsacc">
				<div id="contacc-1">
					<p>Более 50 тысяч предложений на первичном и вторичном рынке.</p>
					<p>Ипотечные программы от 10 ведущих банков.</p>
					<p>Высокий уровень сервиса и индивидуальный подход.</p>
					<p>Предоставление информации удобным способом: SMS, электронная почта, социальные сети, WhatsApp, Viber, Skype.</p>
				</div>
				<div class="notshow" id="contacc-2">
					<p>Полное правовое сопровождение сделки и проверка юридической чистоты объекта.</p>
					<p>Тщательное соблюдение стандартов работы.</p>
					<p>Безупречная репутация и известный бренд компании.</p>
				</div>
				<div class="notshow" id="contacc-3">
					<p>Бесплатное проживание в апарт-отеле.</p>
					<p>Предпродажная подготовка вашей квартиры за счет компании.</p>
					<p>Профессиональная фотосъемка вашей недвижимости.</p>
					<p>Предоставление водителей на комфортных автомобилях бизнес-класса для показов объектов, сопровождения на сделку, встречи в аэропорту или вокзале иногородних клиентов.</p>
				</div>
				<div class="notshow" id="contacc-4">
					<p>Недвижимость в новостройках по ценам застройщиков, без комиссии.</p>
					<p>Срочный выкуп квартир на вторичном рынке.</p>
					<p>Выкуп квартиры у застройщика в пользу клиента.</p>
					<p>Кредитование  под залог квартиры.</p>
				</div>
			</div>
		</div>
	</div>
...
<div id="askLand" class="mfp-hide white-popup-block askpopup" data-name="Переуступки | <?=$rows->header;?>" data-link="<?=getUrl();?>">
	...
	<form id="land_form_ask">
		<div class="rowd">
			<textarea id="comment" data-question="Меня интересует объект по адресу «<?php echo $rows->header; ?>»<?php echo $rows->price_text; ?>. Расскажите, пожалуйста, мне побольше об этом объекте.">Меня интересует объект по адресу «<?php echo $rows->header; ?>»<?php echo $rows->price_text; ?>. Расскажите, пожалуйста, мне побольше об этом объекте.</textarea>
		</div>
		<div class="rowd">
		...
		<div class="rowd">
			<input type="submit" class="submit-ask" name="submit" value="отправить" onclick="yaCounter29760432.reachGoal('otpravit-vse-zayav'); _gaq.push(['_trackEvent', 'OtpravitVoprosPereustupki', 'Click']); return true;" />
		</div>
	</form>
</div>
