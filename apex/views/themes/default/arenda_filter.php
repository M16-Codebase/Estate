...
$inmetro = $this->metro_model->getAllFilterArenda();

$this->load->model('rayon/rayon_model','rayon_model');
$rayons = $this->rayon_model->getAllFilterArenda();
...
if(empty($params['csrok'])) { $params['csrok'] = '0'; } else {$csrok_array = explode(",", $params['csrok']);}
...
if (!empty($params['adress']) || $params['square_from'] > $filter->minMaxS['min'] ||
    $params['square_to'] < $filter->minMaxS['max'] || !empty($params['metro']) ||
    !empty($params['csrok']) || !empty($params['rooms'])) $long = 1;
    ...
    <div class="contain">
        <div class="mainfilter">
            <div class="filterchoose clearfix">
                <?php echo $this->load->view(getPath(__FILE__,'/parth_filter_menu'), array('current' => 'arenda'), TRUE); ?>
            </div>
            <div class="filtercont">
                <div class="filter-display active" id="cont-1">

                    <form method="get" class="arenda-form" action="/arenda">
                            ...
                            <div class="short-filter clearfix">
                                <div class="scolumn fcolumn">
                                    <div class="clearfix one-filter-price">
                                        <label>стоимость <span>(тыс.руб)</span></label>
                                        <div class="filter-slider-container">
                                            <div id="price-slider-arenda" class="filter-slider range"></div>
                                        </div>
                                        <div class="prdiv price-b" data-min="<?php echo $filter->minMax['min']; ?>" data-max="<?php echo $filter->minMax['max']; ?>">
                                            <span class="sq_uch">от </span>
                                            <input type="text" name="price_from" id="arPrice-from" class="price_input" value="<?php echo $params['price_from']; ?>" />
                                            <span class="sq_uch"> до </span>
                                            <input type="text" name="price_to" id="arPrice-to" class="price_input" value="<?php echo $params['price_to']; ?>" />
                                        </div>
                                    </div>
                                    <div class="clearfix one-filter-price long-filter mtop30">
                                        <label>площадь <span>(м<sup>2</sup>)</span></label>
                                        <div class="filter-slider-container">
                                            <div id="square-slider-arenda" class="filter-slider range"></div>
                                        </div>
                                        <div class="prdiv square-b" data-min="<?php echo $filter->minMaxS['min']; ?>" data-max="<?php echo $filter->minMaxS['max']; ?>">
                                            <span class="sq_uch">от </span>
                                            <input type="text" name="square_from" id="arSquare-from" class="price_input" value="<?php echo $params['square_from']; ?>" />
                                            <span class="sq_uch"> до </span>
                                            <input type="text" name="square_to" id="arSquare-to" class="price_input" value="<?php echo $params['square_to']; ?>" />
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="lcolumn fcolumn">
                                    <div class="clearfix one-filter wd235">
                                        <input type="hidden" name="srok" value="<?=$params['srok']?>" class="sel_arenda_srok" />
                                        <select class="marenda_srok" multiple="multiple">
                                            <?php foreach($filter->srok as $k=>$r): ?>
                                                <option value="<?php echo $k; ?>" <?php if($params['srok'] && in_array($k, $srok_array)) echo 'selected'; ?>><?php echo $r; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    <div class="clearfix one-filter wd235 long-filter">
                                        <input type="hidden" name="csrok" value="<?=$params['csrok']?>" class="sel_arenda_сsrok" />
                                        <select class="marenda_csrok" multiple="multiple">
                                            <?php foreach($filter->count_srok as $k=>$r): ?>
                                                <option value="<?php echo $k; ?>" <?php if($params['csrok'] && in_array($k, $csrok_array)) echo 'selected'; ?>><?php echo $r; ?></option>
                                            <?php endforeach; ?>
                                        </select>
                                    </div>
                                    ...
                                    <div class="clearfix one-filter">
                                        ...
                                        <div class="btn-group chk-btn" data-toggle="buttons">
                                            <label class="btn btn-primary<?php if(in_array(1, $room_array)) echo ' active'; ?>">
                                                <input type="checkbox" value="1" <?php if(in_array(1, $room_array)) echo 'checked'; ?> class="arenda-check">Комната
                                            </label>

                                            <label class="btn btn-primary<?php if(in_array(2, $room_array)) echo ' active'; ?>">
                                                <input type="checkbox" value="2" <?php if(in_array(2, $room_array)) echo 'checked'; ?> class="arenda-check">Квартира
                                            </label>

                                            <label class="btn btn-primary<?php if(in_array(3, $room_array)) echo ' active'; ?>">
                                                <input type="checkbox" value="3,16" <?php if(in_array(3, $room_array)) echo 'checked'; ?> class="arenda-check">Коттедж
                                            </label>
                                        </div>
                                    </div>
                                    <div class="clearfix one-filter wd235 long-filter">
                                        <input type="hidden" name="rooms" value="<?=$params['rooms']?>" class="sel_arenda_rooms" />
                                        <select class="marenda_rooms" multiple="multiple">
...
<!--noindex-->
                    <div id="metro_n_districts" class="mfp-hide white-popup-block distromet" data-cat="arenda">
