...
$offer['url']					= 'http://m16-estate.ru/resale/'.$building['link'];	//* URL страницы с объявлением.
...
if ((int)$building['price'] > 0)
{
	...
	fwrite($handle, $this->load->view('part_resale', array('id' => $prefix.$building['id'], 'row' => $arr, 'agent' => $agent), TRUE));
	fclose($handle);
}
