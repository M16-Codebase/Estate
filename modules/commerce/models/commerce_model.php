...
class commerce_model extends MY_Model
{
    ...
    function all_count($banned = false, $where = '', $like = '')
    {
        ...
        $this->db->where('buildings.commerce','1'); // вытаскивать только видимые данные
        $this->db->where('buildings.commerce_banned','0'); // вытаскивать только видимые данные

        // Новострой или вторичка
        if($get_active_tab == 'buildings') {
            $this->db->where('buildings.razdelu', 4);
        } else {
    ...
    **/
	function pagination($offset = 0, $uri, $params)
    {
        ...
        $this->db->order_by('buildings.commerce_sort', 'desc'); // сортировка
