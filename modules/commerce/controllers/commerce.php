...
class commerce extends MY_Controller {
    ...
    function __construct()
	{
        ...
        include(MDPATH.'commerce/moduleinfo.php');
        ...
        if(empty($get_active_tab)) {
            $this->session->set_userdata('commerce_active_tab', 'buildings');
        }
	}
	...
	{		           
		...
		$data['text_bottom'] = shortcodes_db('text_bottom_commerce');
		$data['text_top'] = shortcodes_db('text_top_commerce');
        ...
        if($dataRow) // проверяем есть ли данные
        {
            ...
            foreach($dataRow as $key=>$v)
            {
                ...
                //}
                $data['rows']->$key->deadline = $parent_idtypesdelka[$v['typesdelka_id']]; //$this->dataKv($v['korpus_value']);
                ...
                elseif ($v['price_arenda'] > 0){
                    $price .= '<span>'.$this->number_format_drop_zero_decimals((($v['price_arenda'])/1000000), 1).'</span> млн руб';
                }
	...
	**/
    ...
    function _getFilterBuildings()
    {
        ...
        $this->load->model('commerce/'.$model, $model);
        ...
        if($get_active_tab == 'buildings') {
            return $this->load->view('themes/' . $this->defaultTheme . '/' . 'commerce_filter_2', $data, true);
        } else {
            return $this->load->view('themes/' . $this->defaultTheme . '/' . 'resale_filter_commerce', $data, true);
        }
    }
    ...
    {
        $this->session->set_userdata('commerce_active_tab', $id);
        redirect('commerce');
    }
