...
class Index extends MY_Admin
{
	...
	function index($all = '')
	{
		...
		if(!empty($return_data)) {
			foreach($return_data as $val) {
				...
				$ch = 'checked';
				if($val['commerce_banned'] == 1) {
					$ch = '';
				}

				$dani = "
                    <input type=\"checkbox\" id=\"switch_{$val['id']}\" class=\"switch switches mini wide\" data-id=\"{$val['id']}\" value=\"\" {$ch}>
                    <span style='display: none;'>{$val['commerce_banned']}</span>
                ";

                $sort = "
                            <input style=\"width: 100%;\" value=\"{$val['commerce_sort']}\" data-action=\"commerce_sort\" data-id=\"{$val['id']}\" class=\"sorte\" />
        ...
        $listData['captionTable'] = 'Коммерция';
    ...
    function config()
    {
        $conf = $this->load->config('commerce/commerce', true);
