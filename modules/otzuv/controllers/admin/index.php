    ...
    **/
    ...
    {        
        ...
        if($this->input->post('dani'))
        {
           ...
           $mas['otzuv_key'] = $this->$model->createKey($mas['name'], $mas['date']);
            $statusLink = false;
            unset($mas['foto[]']);
            unset($mas['video[]']);
            unset($mas['fotodesc']);
    ...
    **/
    ...
    {        
        ...
        {                        
            ...
            {
                // Грузим модуль админа и вызываем нужную функцию для формирования вида
                $mas['date'] = date('d.m.Y', $mas['date']);
                $listData = $this->load->module('admin')->add_edit_data($this->table, $this->module, $mas, $this->ValidPolya, $this->paramFields($id, $mas['audio']), '');
    ...
    function parseJson($data)
    {
        if(is_array($data))
        {
            foreach($data as $d)
            {
                ...
                elseif($d['name'] == 'fotodesc[]')
                {
                    $alt[] = $d['value'];
                }
                elseif($d['name'] == 'fotosort[]')
                {
                    $sort[] = $d['value'];
                }
            ...
            if(isset($video)) {
                ...
                $sds['video_count'] = count($video);
                $sds['video'] = serialize($serailizev);
            }
            else {
                $sds['video_count'] = 0;
            }
            if(isset($audio)) {
                $sds['audio_count'] = count($audio);
                $sds['audio'] = implode(',',$audio);
            }
            else {
                $sds['audio_count'] = 0;
            }
...
/** --------------------------------------------------------------------------------------------- **/
    ...
    **/
    function paramFields($pid=null,$audio=null)
    {    
        ...
        $audios = array();
        if (!empty($audio)){
            $this->load->model('files_table/admin_files_table_model','admin_files_table_model');
            $audioss = $this->admin_files_table_model->module_get_in($audio);
            foreach($audioss as $p)
            {
                $audios_id[] = $p['id'];
                $audios_name[] = $p['orig_name'];
            }
            $serailize = array(
                'audio' => $audios_id,
                'name' => $audios_name
            );
            $audios = serialize($serailize);
        }

        $this->load->model('tags/admin_tags_model','tags_model');
        $tags = $this->tags_model->parent_id();
        foreach($tags as $p)
        {
            $tags_list[$p['id']] = $p['name'];
        }
