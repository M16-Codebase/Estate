...
class admin_special_model extends CI_Model
{
	// конструктор
...
/** ********************************************************************************************************************************** **/    
	...
	**/
    ...
    public function addSpecials($array, $identity){

        if ($array['special'] == 0 || $array['banned'] == 1){
            if($id = $this->isSpecial($array['parent_id'], $identity)){
    ...
    public function deleteSpecials($parent, $identity){
        if($id = $this->isSpecial($parent, $identity)){
            $this->module_delete($id);
        }
    }

    public function isSpecial($id, $identity){
	...
	public function synchronization($id, $identity, $price){
		if ($ids = $this->isSpecial($id, $identity)){
