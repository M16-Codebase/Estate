...
class admin_shortcode_model extends CI_Model
{
	// конструктор
...
/** ********************************************************************************************************************************** **/    
    ...
    **/ 
    ...
    /**
    * Сохранение шорткодов в файл (так вывод делается в разы быстрее)
    **/
    public function write_shortcode()
    {
        ...
        $this->db->order_by('sort','asc');
        $foreach = $this->db->get($this->table)->result_object();
        foreach($foreach as $dat) 
        {
            $set[] = "\t".'$config["' . $dat->name . '_en"] = "' . htmlspecialchars($dat->text_en) . '";';
            $set[] = "\t".'$config["' . $dat->name . '"] = "' . htmlspecialchars($dat->text) . '";';
        }
        
        $set[] = "\t".'$config[""] = "";';
        
        $set[] = "\n\n/* End of file shortcode.php */";        
        
        $output = implode("\n", $set);
    
        if(write_file(MDPATH . "admin/config/shortcode.php", $output))
        {
            return true;
        }
        else
