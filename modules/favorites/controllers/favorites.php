...
class favorites extends MY_Controller {     
	...
	**/
    ...
    function showHide()
    {
        ...
        $dataRow = $this->$model->getFavorites($favorites);

        $data['data'] = array();
		//echo'sdasdadasdaws';
		//echo'<div style="display:none"><pre>';print_r($dataRow);echo'</pre></div>';
        foreach ($dataRow as $k => $v){
            ...
            $row['name'] = $v['name'];
            $row['price'] = $v['price'];
            $row['link'] = BASEURL.'/'.$v['category'].'/'.$v['link'];
            $row['foto'] = $v['foto'];
            $data['data'][] = $row;
        }

        $this->load->view('themes/'.$this->defaultTheme.'/tmpl/'.'favorites', $data);
    }
    ...
    function showSeen()
    {
        ...
        if ($this->session->userdata('seen')){
            $sess = $this->session->userdata('seen');
            if (count($sess) > 0){
                $dataRow = $this->$model->getFavorites($sess);
            }
        ...
        $this->load->view('themes/'.$this->defaultTheme.'/tmpl/'.'seen', $data);
    }
