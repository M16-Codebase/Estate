...
class admin_interest_model extends CI_Model
{
	// конструктор
...
/** ********************************************************************************************************************************** **/    
	...
	**/
    ...
    public function addInterest($array, $identity){

        if ($array['interest'] == 0 || $array['banned'] == 1){
            $id = $this->isInterest($array['parent_id'], $identity);
            if(!empty($id)){
                $this->module_delete($id);
            }
        }
        else {
            $array['identity'] = $identity;
            if($id = $this->isInterest($array['parent_id'], $identity)){
                unset($array['interest']);
                $this->module_edit($id,$array);
            }
            else {
                unset($array['interest']);
                $this->module_add($array);
            }
        }
    }

    public function deleteInterest($parent, $identity){
        if($id = $this->isInterest($parent, $identity)){
            $this->module_delete($id);
        }
    }

    public function isInterest($id, $identity){
	...
	public function synchronization($id, $identity, $price){
		if ($ids = $this->isInterest($id, $identity)){
			$this->module_edit($ids,array('price'=>$price));
		}
	}
