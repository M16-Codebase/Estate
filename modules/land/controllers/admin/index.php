    ...
    **/
    ...
    {        
        ...
        if($this->input->post('dani'))
        {
            ...
            $types = array();
            if (!empty($mas['sdelka_id'])){
                $types = explode(",", $mas['sdelka_id']);
            }
...
/** --------------------------------------------------------------------------------------------- **/
    ...
    **/
    ...
    {
        $this->load->model('land/admin_land_model','land_model');
        $lands = $this->land_model->getLikes($pid);
        foreach($lands as $p)
        {
            $land_likes[$p['id']] = $p['name'];
        }

        // Массив параметров для полей
        $this->load->model('land_rayon/admin_land_rayon_model','admin_land_rayon_model');
        $parentrayon = $this->admin_land_rayon_model->parent_id();
        ...
        {
            $parent_idsdelka[$p['id']] = $p['name'];
        }
        $this->load->model('land_type/admin_land_type_model','admin_land_type_model');
        $parenttype = $this->admin_land_type_model->parent_id();
        $parent_idtype['0'] = ' -- не выбрано -- ';
