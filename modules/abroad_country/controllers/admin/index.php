            ...
            {                                            
                ...
                {                    
                    ...
                    if($this->needRoute)
                    {
                        ...
                        $array_buildings = $this->routeArray($mas['link'], $mas['name'],'abroad');
                        $this->mdRoute->module_add($array_buildings);
                        $array_buildings_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'abroad');
            ...
            {                                            
                ...
                {                    
                    ...
                    if($this->needRoute)
                    {
                        ...
                        //buildings metro route
                        $array_buildings = $this->routeArray($mas['link'], $mas['name'],'abroad');
                        if($this->mdRoute->module_getValueLink('abroad/'.$currentLink['link']))
                        {
                            $this->mdRoute->module_editLink('abroad/'.$currentLink['link'], $array_buildings);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_buildings);
                        ...
                        $array_buildings_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'abroad');
                        if($this->mdRoute->module_getValueLink('abroad/'.$currentLink['link'].'/(:num|all)'))
                        {
                            $this->mdRoute->module_editLink('abroad/'.$currentLink['link'].'/(:num|all)', $array_buildings_pagination);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_buildings_pagination);
                ...
                if(count($checked) > 0)  // проверка на то или массив не пустой и имеет более 1-й записи
                {
                    ...
                    {                    
                        ...
                        {                                                                
                            ...
                            {
                                $this->mdRouter->module_deleteLink('abroad/'.$chDel[1]);
                                $this->mdRouter->module_deleteLink('abroad/'.$chDel[1].'/(:num|all)');;
                            }
        ...
        else // если переменная с идентификатором не является массивом
        {
            if(!empty($checked) and is_numeric($checked)) // проврка пустоту и является ли цифрой переданный идентификатор
            {
                ...
                if($this->model->module_delete($checked)) // удаляем запись
                {
                    ...
                    {
                        $this->mdRouter->module_deleteLink('abroad/'.$cLink);
                        $this->mdRouter->module_deleteLink('abroad/'.$cLink.'/(:num|all)');
                    }
    ...
    **/
    ...
    function routeArrayPagination($link, $name, $base = '')
    {
        return  array(
            ...
            'value'=>'abroad/'.$this->module.'/'.$link.'/?page=$1',
            'desc'=>$name
        );
    }
