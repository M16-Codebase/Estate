...
/** ********************************************************************************************************************************** **/
    ...
    **/
     ...
     *
     * @param array $where
     * точные значения должны передаваться под ключем 'exact_values'
     * логические под ключем 'logiс_values'
     * @example $where = ['exact_values' => ['id' => 1], 'logiс_values' => 'name IS NOT NULL']
     */
    public function getNews($select, $where)
	{
        if (is_array($select)) {
        ...
        if (is_array($where)) {
            if (isset($where['exact_values']))
                $this->db->where($where['exact_values']);

			if (isset($where['logiс_values'])) {
				if (is_array($where['logiс_values'])) {
					foreach ($where['logiс_values'] as $value) {
						$this->db->where($value, NULL, FALSE);
					}
				} else {
					$this->db->where($where['logiс_values'], NULL, FALSE);
				}
			}
        }
        $query = $this->db->get($this->table);
