	...
	{                                
        ...
        $fields = array(                                
            'sort' => array(
                ...
                'title'=>'<span class="underline with-tooltip blue" title="'.$this->alang->admin_sorting.'">№</span>'
                ),
            'name' => array(
                'width'=>'', 
                'class'=>'',
    ...
    **/
    ...
    {        
        ...
        if($this->input->post('dani'))
        {
            ...
            $mas['link'] = $mas[$mas['type_link']];            
            unset($mas['link_http'], $mas['link_site'], $mas['link_page'], $mas['link_module']);            
        ...
        {                        
            ...
            $listData['table']['parent_id']['params'] = $parents; //option_tree(create_children($parent), '');            
     
            $listData['table']['link_page'] = array(
                            'name' => 'link_page', // имя
                            'id' => 'link_page', // идентификатор
                            'type' => 'input_enum', // тип поля
                            'label' => $this->alang->admin_90, // название поля для <label></label>
                            'value' => '', // значение по умолчанию
                            'params' => $this->$model->moduleLoad(), // параметры
                            'options' => array(
                                    'label_class' => 'hidden blue'
                                )
                        );
            
            $listData['table']['link_site'] = array(
                            'name' => 'link_site', // имя
                            'id' => 'link_site', // идентификатор
                            'type' => 'input', // тип поля
                            'label' => $this->alang->admin_89, // название поля для <label></label>
                            'value' => '/', // значение по умолчанию
                            'params' => $this->$model->moduleLoad(), // параметры
                            'options' => array(
                                    'label_class' => 'hidden blue'
            ...
            $listData['table']['link_http'] = array(
                            'name' => 'link_http', // имя
                            'id' => 'link_http', // идентификатор
                            'type' => 'input', // тип поля
                            'label' => $this->alang->admin_88.' http://', // название поля для <label></label>
                            'value' => 'http://', // значение по умолчанию
                            'params' => $this->$model->moduleLoad(), // параметры
                            'options' => array(
                                    'label_class' => 'hidden blue'
                                )
    ...
    {        
        ...
        {                        
            ...
            if($mas != '') // проверяем если ли данные по такому идентификатору
            {
                ...
                if($mas['type_link'] == 'link_http')
                    { $lh = $mas['link']; }
                elseif($mas['type_link'] == 'link_page')
                    { $lp = $mas['link']; }
                elseif($mas['type_link'] == 'link_site')
                    { $ls = $mas['link']; }
                ...
                $listData['table']['link_page'] = array(
                                ...
                                'type' => 'input_enum', // тип поля
                                'label' => $this->alang->admin_90, // название поля для <label></label>
                                'value' => $lp, // значение по умолчанию
                                'params' => $this->$model->moduleLoad(), // параметры
                                'options' => array(
                                        'label_class' => 'hidden blue'
                                    )
                ...
                $listData['table']['link_site'] = array(
                                ...
                                'type' => 'input', // тип поля
                                'label' => $this->alang->admin_89, // название поля для <label></label>
                                'value' => $ls, // значение по умолчанию
                                'params' => $this->$model->moduleLoad(), // параметры
                                'options' => array(
                                        'label_class' => 'hidden blue'
                                    )
                ...
                $listData['table']['link_http'] = array(
                                ...
                                'type' => 'input', // тип поля
                                'label' => $this->alang->admin_88.' http://', // название поля для <label></label>
                                'value' => $lh, // значение по умолчанию
                                'params' => $this->$model->moduleLoad(), // параметры
                                'options' => array(
                                        'label_class' => 'hidden blue'
                                    )
...
/** --------------------------------------------------------------------------------------------- **/
    ...
    **/
    ...
    {            
        $region = array(
            'header' => '<span class="tag red-bg">Главное меню</span>'
        );
        
        // Тип ссылки            
        $type_link = array(
                '0' => $this->alang->admin_94,                
                'link_page' => $this->alang->admin_95,              
                'link_site' => $this->alang->admin_96,
                'link_http' => $this->alang->admin_97
            );
        
        // Вытаскиваем родителей
        $this->load->model('menu/admin_menu_model','admin_menu_model');                        
        $menu = $this->admin_menu_model->parent_id();                
        foreach($menu as $p)
        {
            $menu_id[$p['id']] = $p['name'];
        }
