...
{
    private $needRoute = true; // нужно ли использовать роутер
	...
	function __construct()
	{
        ...
        include(MDPATH.'rayon/moduleinfo.php');                
            ...
            {                                            
                ...
                {                    
                    ...
                    if($this->needRoute)
                    {
                        ...
                        //buildings metro route
                        $array_buildings = $this->routeArray($mas['link'], $mas['name'],'buildings');
                        $this->mdRoute->module_add($array_buildings);
                        $array_buildings_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'buildings');
                        $this->mdRoute->module_add($array_buildings_pagination);
                        //resale metro route
                        $array_resale = $this->routeArray($mas['link'], $mas['name'],'resale');
                        $this->mdRoute->module_add($array_resale);
                        $array_resale_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'resale');
                        $this->mdRoute->module_add($array_resale_pagination);
                        //residential metro route
                        $array_residential = $this->routeArray($mas['link'], $mas['name'],'residential');
                        $this->mdRoute->module_add($array_residential);
                        $array_residential_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'residential');
                        $this->mdRoute->module_add($array_residential_pagination);
                        //elite metro route
                        $array_elite = $this->routeArray($mas['link'], $mas['name'],'elite');
                        $this->mdRoute->module_add($array_elite);
                        $array_elite_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'elite');
                        $this->mdRoute->module_add($array_elite_pagination);
                        //commercial metro route
                        $array_commercial = $this->routeArray($mas['link'], $mas['name'],'commercial');
                        $this->mdRoute->module_add($array_commercial);
                        $array_commercial_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'commercial');
                        $this->mdRoute->module_add($array_commercial_pagination);
                    }
            ...
            {                                            
                ...
                {                    
                    ...
                    if($this->needRoute)
                    {
                        ...
                        //buildings metro route
                        $array_buildings = $this->routeArray($mas['link'], $mas['name'],'buildings');
                        if($this->mdRoute->module_getValueLink('buildings/'.$currentLink['link']))
                        {
                            $this->mdRoute->module_editLink('buildings/'.$currentLink['link'], $array_buildings);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_buildings);
                        ...
                        $array_buildings_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'buildings');
                        if($this->mdRoute->module_getValueLink('buildings/'.$currentLink['link'].'/(:num|all)'))
                        {
                            $this->mdRoute->module_editLink('buildings/'.$currentLink['link'].'/(:num|all)', $array_buildings_pagination);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_buildings_pagination);
                        ...
                        //resale metro route
                        $array_resale = $this->routeArray($mas['link'], $mas['name'],'resale');
                        if($this->mdRoute->module_getValueLink('resale/'.$currentLink['link']))
                        {
                            $this->mdRoute->module_editLink('resale/'.$currentLink['link'], $array_resale);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_resale);
                        ...
                        $array_resale_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'resale');
                        if($this->mdRoute->module_getValueLink('resale/'.$currentLink['link'].'/(:num|all)'))
                        {
                            $this->mdRoute->module_editLink('resale/'.$currentLink['link'].'/(:num|all)', $array_resale_pagination);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_resale_pagination);
                        ...
                        //residential metro route
                        $array_residential = $this->routeArray($mas['link'], $mas['name'],'residential');
                        if($this->mdRoute->module_getValueLink('residential/'.$currentLink['link']))
                        {
                            $this->mdRoute->module_editLink('residential/'.$currentLink['link'], $array_residential);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_residential);
                        ...
                        $array_residential_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'residential');
                        if($this->mdRoute->module_getValueLink('residential/'.$currentLink['link'].'/(:num|all)'))
                        {
                            $this->mdRoute->module_editLink('residential/'.$currentLink['link'].'/(:num|all)', $array_residential_pagination);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_residential_pagination);
                        ...
                        //elite metro route
                        $array_elite = $this->routeArray($mas['link'], $mas['name'],'elite');
                        if($this->mdRoute->module_getValueLink('elite/'.$currentLink['link']))
                        {
                            $this->mdRoute->module_editLink('elite/'.$currentLink['link'], $array_elite);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_elite);
                        ...
                        $array_elite_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'elite');
                        if($this->mdRoute->module_getValueLink('elite/'.$currentLink['link'].'/(:num|all)'))
                        {
                            $this->mdRoute->module_editLink('elite/'.$currentLink['link'].'/(:num|all)', $array_elite_pagination);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_elite_pagination);
                        ...
                        //commercial metro route
                        $array_commercial = $this->routeArray($mas['link'], $mas['name'],'commercial');
                        if($this->mdRoute->module_getValueLink('commercial/'.$currentLink['link']))
                        {
                            $this->mdRoute->module_editLink('commercial/'.$currentLink['link'], $array_commercial);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_commercial);
                        ...
                        $array_commercial_pagination = $this->routeArrayPagination($mas['link'], $mas['name'],'commercial');
                        if($this->mdRoute->module_getValueLink('commercial/'.$currentLink['link'].'/(:num|all)'))
                        {
                            $this->mdRoute->module_editLink('commercial/'.$currentLink['link'].'/(:num|all)', $array_commercial_pagination);
                        }
                        else
                        {
                            $this->mdRoute->module_add($array_commercial_pagination);
                ...
                if(count($checked) > 0)  // проверка на то или массив не пустой и имеет более 1-й записи
                {
                    ...
                    {                    
                        ...
                        {                                                                
                            ...
                            {
                                $this->mdRouter->module_deleteLink('buildings/'.$chDel[1]);
                                $this->mdRouter->module_deleteLink('resale/'.$chDel[1]);
                                $this->mdRouter->module_deleteLink('buildings/'.$chDel[1].'/(:num|all)');
                                $this->mdRouter->module_deleteLink('resale/'.$chDel[1].'/(:num|all)');
                                $this->mdRouter->module_deleteLink('elite/'.$chDel[1]);
                                $this->mdRouter->module_deleteLink('elite/'.$chDel[1].'/(:num|all)');
                                $this->mdRouter->module_deleteLink('residential/'.$chDel[1]);
                                $this->mdRouter->module_deleteLink('residential/'.$chDel[1].'/(:num|all)');
                                $this->mdRouter->module_deleteLink('commercial/'.$chDel[1]);
                                $this->mdRouter->module_deleteLink('commercial/'.$chDel[1].'/(:num|all)');
                            }
        ...
        else // если переменная с идентификатором не является массивом
        {
            if(!empty($checked) and is_numeric($checked)) // проврка пустоту и является ли цифрой переданный идентификатор
            {
                ...
                if($this->model->module_delete($checked)) // удаляем запись
                {
                    ...
                    {
                        $this->mdRouter->module_deleteLink('buildings/'.$cLink);
                        $this->mdRouter->module_deleteLink('resale/'.$cLink);
                        $this->mdRouter->module_deleteLink('buildings/'.$cLink.'/(:num|all)');
                        $this->mdRouter->module_deleteLink('resale/'.$cLink.'/(:num|all)');
                        $this->mdRouter->module_deleteLink('residential/'.$cLink);
                        $this->mdRouter->module_deleteLink('residential/'.$cLink.'/(:num|all)');
                        $this->mdRouter->module_deleteLink('elite/'.$cLink);
                        $this->mdRouter->module_deleteLink('elite/'.$cLink.'/(:num|all)');
                        $this->mdRouter->module_deleteLink('commercial/'.$cLink);
                        $this->mdRouter->module_deleteLink('commercial/'.$cLink.'/(:num|all)');
                    }
    ...
    **/
    function parseJson($data)
    {
        if(is_array($data))
        {
            ...
            //if(!isset($sds['banned'])) { $sds['banned'] = 1; } else { $sds['banned'] = 0; }
        }
        else
        {
...
/** --------------------------------------------------------------------------------------------- **/
     ...
     *  Роутер
    **/
    function routeArray($link, $name, $base = '')
    {
        return  array(
            'key'=>$base.'/'.$link,
            'value'=>'buildings/'.$this->module.'/'.$link,
            'desc'=>$name
        );
    }

    function routeArrayPagination($link, $name, $base = '')
    {
        return  array(
            'key'=>$base.'/'.$link.'/(:num|all)',
            'value'=>'buildings/'.$this->module.'/'.$link.'/?page=$1',
            'desc'=>$name
        );
    }
