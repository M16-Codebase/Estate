    ...
    **/
    ...
    {        
        ...
        if($this->input->post('dani'))
        {
            ...
            if(!$statusLink)
            {
                $mas['link'] = toTranslitUrl($mas['name']);
                if($this->$model->module_add($mas))
                {                    
                    $id = $this->db->insert_id();
                    $statusPage = $statusPageLink.$id;
                    $this->$model->module_edit($id,array('link'=>$mas['link'].'-'.$id));
            ...
            else
            {
                ...
                if($this->$model->module_add($mas))
                {
                    ...
                    $newLink = toTranslitUrl($mas['name']).'-'.$ids;
    ...
    **/
    ...
    {        
        ...
        if($this->input->post('dani')) // ловим данные
        {
            ...
            $mas['link'] = toTranslitUrl($mas['name']).'-'.$id;
            if(isset($mas['link']))
            {
    ...
    **/
    function parseJson($data)
    {
        if(is_array($data))
        {
            ...
            if(isset($sds['is_house'])) { if($sds['is_house'] == 0) $sds['is_house'] = 1; }
            if(!isset($sds['is_house'])) { $sds['is_house'] = 0; }
        }
        else
        {
...
/** --------------------------------------------------------------------------------------------- **/
    ...
    **/
    ...
    {    
        ...
        $parent_id = array();
        $this->load->model('buildings/admin_buildings_model','admin_buildings_model');
        $parent = $this->admin_buildings_model->getCottages();
        ...
        return array(
                        'parent_id' => $parent_id,
                        'is_house' => array($this->alang->admin_72,$this->alang->admin_73),
