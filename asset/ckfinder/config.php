...
$config['ResourceType'][] = Array(
        ...
        /*if($_SESSION['CKFinder_UserRole'] != 'super' && $_SESSION['CKFinder_UserRole'] != 'admin')
        {
            $paths = '/users/'.$_SESSION['CKFinder_UserRole'].'/';
            if(!file_exists($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/images/users'))
            {
                mkdir($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/images/users');                                        
            }
            if(!file_exists($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/_thumbs/images/users'))
            {
                mkdir($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/_thumbs/images/users');
            }
            if(!file_exists($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/images/users'.$_SESSION['CKFinder_UserRole']))
            {
                mkdir($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/images/users'.$_SESSION['CKFinder_UserRole']);                        
                mkdir($_SERVER["DOCUMENT_ROOT"].'/asset/uploads/_thumbs/images/users'.$_SESSION['CKFinder_UserRole']);
            }
        }    */

$config['ResourceType'][] = Array(
		'name' => 'Images',
		'url' => $baseUrl . 'images',//.$paths,
		'directory' => $baseDir . 'images',//.$paths,
		'maxSize' => 0,
...
/*
Если установлено в true то идет проверка размера изображения
*/
