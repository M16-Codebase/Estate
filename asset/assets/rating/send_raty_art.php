...
if(isset($_POST['url'])){
	...
	$raty=sqlGet('ci_news','`rating`,`raters`',"`link`='$link[4]'");
	//print_r($raty);
	$rating= floor(($raty['rating']))*20;  //$raty['rating']/$raty['raters']
	echo $rating;
}else{
	$link=explode('/',$_POST['uri']);
	$rat=(int)$_POST['rating'];
	if(sqlUpdate('ci_news',"`rating`=`rating`+$rat,`raters`=`raters`+1","`link`='$link[4]'")){
